    ...
    while (i < todo.length) {
      ...
      if (placeholder instanceof Node) {
        ...
        for (let j = 0; j < array.length; j++) {
          ...
          if (
            maybeImp instanceof ImportsName &&
            forceDefaultImport.includes(maybeImp.source)
          ) {
            const name = getName(maybeImp.source);
            array[j] = code`${new ImportsDefault(name, maybeImp.source)}.${
              maybeImp.sourceSymbol || maybeImp.symbol
            }`;
          } else if (
            maybeImp instanceof ImportsName &&
            ...
            const name = getName(maybeImp.source);
            array[j] = code`${new ImportsAll(name, maybeImp.source)}.${
              maybeImp.sourceSymbol || maybeImp.symbol
            }`;
          } else if (
            maybeImp instanceof ImportsDefault &&
            forceModuleImport.includes(maybeImp.source)
          ) {
...
export function deepGenerate(
  used: ConditionalOutput[],
  object: unknown
): string {
  let result = "";
...
function assignAliasesIfNeeded(
  defs: Def[],
  imports: Import[],
  ...
  imports.forEach((i) => {
    if (
      ...
      !(
        sameModule(i.source, ourModulePath) ||
        (i.definedIn && sameModule(i.definedIn, ourModulePath))
      )
    ) {
...
function maybePretty(input: string, options?: DPrintOptions): string {
  try {
    return dprint.format("file.ts", input.trim(), {
      ...baseOptions,
      ...options,
    });
  } catch (e) {
