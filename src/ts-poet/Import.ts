...
export class ImportsName extends Imported {
  ...
  public toImportPiece(): string {
    return this.sourceSymbol
      ? `${this.sourceSymbol} as ${this.symbol}`
      : this.symbol;
  }
}
...
export function emitImports(
  ...
  const augmentImports = groupBy(
    filterInstances(imports, Augmented),
    (a) => a.augmented
  );
  ...
  Object.entries(importsByModule).forEach(([modulePath, imports]) => {
    ...
    unique(filterInstances(imports, ImportsAll).map((i) => i.symbol)).forEach(
      (symbol) => {
        result += `import * as ${symbol} from '${importPath}';\n`;
        const augments = augmentImports[symbol];
        if (augments) {
          augments.forEach(
            (augment) => (result += `import '${augment.source}';\n`)
          );
        }
      }
    );
    ...
    const names = unique(
      allNames.filter((i) => !i.typeImport).map((it) => it.toImportPiece())
    );
    const def = unique(
      filterInstances(imports, ImportsDefault).map((it) => it.symbol)
    );
    ...
    if (names.length > 0 || def.length > 0) {
      ...
      result += `import ${[...defPart, ...namesPart].join(
        ", "
      )} from '${importPath}';\n`;
  ...
  const sideEffectImports = groupBy(
    filterInstances(imports, SideEffect),
    (a) => a.source
  );
  Object.keys(sideEffectImports).forEach(
    (it) => (result += `import '${it}';\n`)
  );
...
export function maybeRelativePath(
  outputPath: string,
  importPath: string
): string {
...
export function sameModule(path1: string, path2: string): boolean {
  ...
  const [basePath1, basePath2] = [path1, path2].map((p) =>
    p.replace(/\.[tj]sx?/, "")
  );
  return (
    basePath1 === basePath2 ||
