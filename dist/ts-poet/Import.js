...
export class Imported extends Import {
...
export function emitImports(imports, ourModulePath, importMappings) {
    if (imports.length == 0) {
        return "";
    }
    let result = "";
    ...
    const importsByModule = groupBy(imports.filter((it) => it.source !== undefined &&
        // Ignore imports that are in our own file
        !(it instanceof ImportsName &&
            it.definedIn &&
            sameModule(it.definedIn, ourModulePath))), (it) => it.source);
...
export function maybeRelativePath(outputPath, importPath) {
...
export function sameModule(path1, path2) {
